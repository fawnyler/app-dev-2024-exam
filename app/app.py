...
from admin_function import admin_bp
app.register_blueprint(admin_bp)

from moder_function import moder_bp
...
def book_view(book_id):
    try:
        ...
        # После получения book из базы данных
        if book:
            # Разделение строки с жанрами на список, если жанры есть
            if book['genre']:
                book['genres_list'] = book['genre'].split(', ')
            else:
                book['genres_list'] = []

            # Получаем рецензии к книге
            cursor = db.cursor(dictionary=True)
            query_reviews = """
                SELECT 
                    reviews.id,
            ...
            # Преобразование текста рецензий из Markdown в HTML
            for review in reviews:
...
def add_review(book_id):
    if request.method == 'POST':
        ...
        try:
            ...
            query_check = "SELECT id FROM reviews WHERE book_id = %s AND user_id = %s"
            ...
            query = """
                INSERT INTO reviews (book_id, user_id, rating, text, date_added)
                VALUES (%s, %s, %s, %s, NOW())
            """
